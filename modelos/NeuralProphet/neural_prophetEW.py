...
def treinar_modelo(n_lags, epochs, batch_size, learning_rate):
    ...
    if rmse_mean < metrics_params_EW["best_rmse"]:
        ...
        metrics_params_EW["best_n_lags"] = n_lags
        metrics_params_EW["best_batch_size"] = batch_size
        metrics_params_EW["best_learning_rate"] = learning_rate

    with open(metrics_file2, "a") as f:
        f.write(
            f"NP-EW,{n_lags},{epochs},{batch_size},{learning_rate},{rmse_mean},{rmse_std},{mae_mean},{mae_std},{mape_mean},{mape_std},{r2_mean},val\n"
            # f"NP-EW,{n_lags},{batch_size},{learning_rate},{rmse_mean},{rmse_std},{mae_mean},{mae_std},{mape_mean},{mape_std},{r2_mean},val\n"
        )
    ...
    ytrue_yhat_df.to_csv(
        os.path.join(
            results_csv_dir,
            f"NP-EW_{n_lags}_{epochs}_{batch_size}_{learning_rate}_{timestamp}.csv",
            # f"NP-EW_{n_lags}_{batch_size}_{learning_rate}_{timestamp}.csv",
        ),
        index=False,
    )
...
with open(metrics_file, "a") as f:
    f.write(
        "NP-EW,{},{},{},{},{},{},{},{},val\n".format(
            metrics_params_EW["best_n_lags"],
            metrics_params_EW["best_epochs"],
            metrics_params_EW["best_batch_size"],
            metrics_params_EW["best_learning_rate"],
...
for i, (trainidxs, testidxs) in enumerate(expanding_window_test.split(df)):
    ...
    model = NeuralProphet(
        ...
        batch_size=metrics_params_EW["best_batch_size"],
        n_lags=metrics_params_EW["best_n_lags"],
        learning_rate=metrics_params_EW["best_learning_rate"],
        n_forecasts=len(y_t),
    )
    ...
    # Itera pelas colunas e adiciona os valores não-nulos na lista
    for col in yhat_columns:
        predicted_values.extend(predictions[col].dropna().tolist())
...
with open(metrics_file, "a") as f:
    f.write(
        f"NP-EW,{metrics_params_EW['best_n_lags']},{metrics_params_EW['best_epochs']},{metrics_params_EW['best_batch_size']},{metrics_params_EW['best_learning_rate']},{rmse_mean},{mae_mean},{mape_mean},{r2_mean},test\n"
    )
