...
metrics_file = os.path.join(output_dir, f"metrics-arima_{timestamp}.csv")

with open(metrics_file, "w") as f:
    f.write("model,order,rmse,mae,mape,r2,split\n")
...
values_file = os.path.join(output_dir2, f"results_arima-{timestamp}.csv")
...
print("Best_Params_SW2Y_Val", best_params_SW2Y)
...
for i, (trainidxs, testidxs) in enumerate(SW2Y_test.split(df)):
    ...
    arima_model = sm.tsa.ARIMA(y, order=best_params_SW2Y)
    arima_fit = arima_model.fit()

    print("Parametros atuais para teste: ", best_params_SW2Y)
...
print("Best_Params_SW1Y_Val", best_params_SW1Y)
